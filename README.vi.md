...
### 🔥 Xem những gì AI thấy (E2E Trace thực tế)

Khác với các công cụ log truyền thống, **Trace2Prompt** chụp lại toàn bộ hành trình của người dùng. Dưới đây là ví dụ về một request `POST /api/orders` phức tạp được công cụ ghi lại, sẵn sàng để gửi cho AI:

<details>
<summary>👉 Click để mở rộng Trace của POST Request thực tế (có Masking & SQL)</summary>
...
<details>
<summary>👉 Click để mở rộng Trace của GET Request thực tế (có Masking & SQL)</summary>
...
> ✅ **Xác minh Backend:**
>
> 1. Khởi động ứng dụng Backend của bạn.
> 2. Thực hiện một yêu cầu API đơn giản đến Backend của bạn (ví dụ: gọi điểm cuối GET).
> 3. Mở giao diện web Trace2Prompt (`http://localhost:4319`). Nếu bạn thấy nhật ký yêu cầu xuất hiện ở đó, tác nhân đã được gắn kết thành công!
...
### Bước 3: Kết nối giao diện người dùng (Browser UI Journey)
...
> ✅ Xác minh: Mở Công cụ dành cho nhà phát triển của trình duyệt (Nhấn F12 -> Bảng điều khiển). Nếu bạn thấy thông báo 🟢 [Trace2Prompt] E2E Sensor FULLY INITIALIZED!, thì giao diện người dùng đã được kết nối thành công.

### Bước 4: Trải nghiệm ma thuật AI!
